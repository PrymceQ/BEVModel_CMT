...
train_pipeline = [
    ...
    dict(type='ResizeCropFlipImage', data_aug_conf = ida_aug_conf, training=True), # 进行了resize，随机的crop
    # 原本图片 HxW = 900x1600, resize后(模型的输入尺寸)为 320x800
...
model = dict(
    ...
    img_backbone=dict(
        ...
        style='pytorch'),
    img_neck=dict(
        type='CPFPN',               # projects/mmdet3d_plugin/models/necks/cp_fpn.py
        in_channels=[1024, 2048],
        out_channels=256,
        num_outs=2),
    pts_voxel_layer=dict(
        # 点云的point是指在3D空间中的单个点
        num_point_features=5,
        max_num_points=10,
        # 体素voxel是指将3D空间划分成一个个的网格
        voxel_size=voxel_size,  # voxel_size = [0.1, 0.1, 0.2]
        max_voxels=(120000, 160000),
        point_cloud_range=point_cloud_range),  # point_cloud_range = [-54.0, -54.0, -5.0, 54.0, 54.0, 3.0]
                ...
                transformerlayers=dict(
                    ...
                    attn_cfgs=[
                        dict(
                            ...
                            dropout=0.1),
                        dict(
                            type='PETRMultiheadAttention', # PETRMultiheadFlashAttention
                            embed_dims=256,
                            num_heads=8,
                            dropout=0.1),
                        ],
...
optimizer = dict(
    type='AdamW',
    lr=0.00007,  # 如果是在4个gpu上的话，lr=0.00007
    paramwise_cfg=dict(
        custom_keys={
...
lr_config = dict(
    policy='cyclic',
    # Relative ratio of the highest LR and the lowest LR to the initial LR.(up_ratio, down_ratio)  
    target_ratio=(3, 0.0001),   # target_ratio=(6, 0.0001) # 将up_ratio=6调整至3
    cyclic_times=1,
    step_ratio_up=0.4)
