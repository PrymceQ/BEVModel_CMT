...
class CmtTransformer(BaseModule):
    ...
    def forward(self, x, x_img, query_embed, bev_pos_embed, rv_pos_embed, attn_masks=None, reg_branch=None):
        ...
        """
        # x (2, 256, 128, 128) #
        # x_img (2, 256, 20, 50) # img_feature
        # query_embed (2, 1540, 256) # num_query=1540
        # bev_pos_embed (128*128, 256)
        # rv_pos_embed (12, 20, 50, 256)
        # attn_masks (1540, 1540)
        bs, c, h, w = x.shape
        bev_memory = rearrange(x, "bs c h w -> (h w) bs c") # (128*128, 2, 256) [bs, n, c, h, w] -> [n*h*w, bs, c]
        rv_memory = rearrange(x_img, "(bs v) c h w -> (v h w) bs c", bs=bs) # (6*20*50, 2, 256)
        bev_pos_embed = bev_pos_embed.unsqueeze(1).repeat(1, bs, 1) # (128*128, 2, 256) [bs, n, c, h, w] -> [n*h*w, bs, c]
        rv_pos_embed = rearrange(rv_pos_embed, "(bs v) h w c -> (v h w) bs c", bs=bs) # (6*20*50, 2, 256)
        
        memory, pos_embed = torch.cat([bev_memory, rv_memory], dim=0), torch.cat([bev_pos_embed, rv_pos_embed], dim=0)
        # memory (128*128 + 6*20*50, 2, 256)
        # pos_embed (128*128 + 6*20*50, 2, 256)

        query_embed = query_embed.transpose(0, 1)  # (1540, 2, 256) [num_query, dim] -> [num_query, bs, dim]

        mask =  memory.new_zeros(bs, memory.shape[0]) # [bs, n, h, w] -> [bs, n*h*w]

        target = torch.zeros_like(query_embed) # (1540, 2, 256) [num_query, dim]
        ...
        out_dec = out_dec.transpose(1, 2) # out_dec
        # out_dec (6, 2, 1540, 256)
