...
class CmtHead(BaseModule):
    ...
    def forward_single(self, x, x_img, img_metas):
        ...
        """
        # x: pts_feats = [(B, 512, 128, 128)]
        # x_img: img_feats = [(B * N, 256, 20, 50), (B * N, 256, 10, 25)]
        # 例 B=2，N=6
        ## 这里遍历了img_feats, 以x_img -> (12, 256, 20, 50)
        ret_dicts = []
        x = self.shared_conv(x) # self.shared_conv = Conv(in_channel=512, out_channel=256, kernal_size=3, padding=1)
        # x -> (2, 256, 128, 128)


        # !!!!!从这里开始半懂半不懂
        reference_points = self.reference_points.weight
        # reference_points = nn.Embedding(900, 3)
        reference_points, attn_mask, mask_dict = self.prepare_for_dn(x.shape[0], reference_points, img_metas)
        
        mask = x.new_zeros(x.shape[0], x.shape[2], x.shape[3]) # (2, 128, 128)
        ...
        # rv_pos_embeds -> (2*6, 20, 50, 256)
        # bev_pos_embeds -> (128 * 128, 256)

        bev_query_embeds, rv_query_embeds = self.query_embed(reference_points, img_metas)
        # rv_query_embeds (2, 1540, 256)
        # bev_query_embeds (2, 1540, 256)
        ...
        for task_id, task in enumerate(self.task_heads, 0):
            outs = task(outs_dec)
            # outs -> dict 记录点云中的单个点的信息
            # 'center' (6, 2, 1540, 2) 中心点坐标信息 xy
            # 'height' (6, 2, 1540, 1) 中心点的高度 h
            # 'dim' (6, 2, 1540, 3) 目标框的长/宽/高
            # 'rot' (6, 2, 1540, 2) 旋转角度 rot_x, rot_y
            # 'vel' (6, 2, 1540, 2) 速度信息 v_x, v_y
            # 'cls_logits' (6, 2, 1540, 10) 类别分数
